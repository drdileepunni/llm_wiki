...
class ReActTracer:
    ...
    def log_tokens(self, input_tokens: int, output_tokens: int, thinking_tokens: int = 0, cached_tokens: int = 0) -> None:
        self._cur["tokens"] = {"in": input_tokens, "out": output_tokens, "thinking": thinking_tokens, "cached": cached_tokens}
        logger.debug(
            "[%s] %s enc=%d round=%d tokens in=%d out=%d thinking=%d cached=%d",
            self.step, self.cpmrn, self.encounter,
            self._cur.get("round", "?"), input_tokens, output_tokens, thinking_tokens, cached_tokens,
        )
    ...
    def save(self, final_output: Any = None) -> None:
        ...
        total_cached  = sum((r.get("tokens") or {}).get("cached", 0)   for r in self.rounds)
        ...
        doc = {
            ...
            "total_tokens": {"in": total_in, "out": total_out, "thinking": total_think, "cached": total_cached},
            "rounds":       self.rounds,
