...
_FLASH_LITE = ("gemini-3.1-flash-lite", 0.25, 1.50)
_FLASH_25   = ("gemini-2.5-flash",      0.30, 2.50)
...
def _pricing(step: str) -> tuple[str, float, float]:
    return _FLASH_25 if step in _FLASH_25_STEPS else _FLASH_LITE


def _cost(step: str, input_tok: int, output_tok: int, thinking_tok: int) -> float:
    """Return total cost in USD for a token count triple."""
    _, input_rate, output_rate = _pricing(step)
    return (
...
def compute_run_cost(db: Any, run_started_at: datetime) -> dict:
    ...
    for step, agg in by_step.items():
        agg["cost_usd"] = round(
            _cost(step, agg["input_tokens"], agg["output_tokens"], agg["thinking_tokens"]), 6
        )
        model, in_r, out_r = _pricing(step)
        pricing_snapshot[step] = {"model": model, "input_per_1m": in_r, "output_per_1m": out_r}
