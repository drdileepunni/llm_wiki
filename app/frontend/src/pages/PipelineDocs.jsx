...
const SYNC_LANES = [
  ...
  { id: 'context', label: 'Context Building · Structured Summary',       color: '#f97316' },
  { id: 'cds',     label: 'CDS · Conditional (worsening / critical)',    color: '#4ade80' },
]

const SYNC_ROW_DEFS = [
  { laneId: 'sync',    nodeIds: ['workspace_in', 'get_admitted', 'chart_pull', 'snapshot_store'] },
  { laneId: 'notes',   nodeIds: ['load_admission', 'build_faiss', 'note_cache'] },
  { laneId: 'context', nodeIds: ['delta_extract', 'summary_update', 'running_ctx'] },
  { laneId: 'cds',     nodeIds: ['worsening_check', 'cds_call', 'notes_tool', 'suggestions', 'gchat_alert'] },
]
...
const SYNC_NODES = [
  ...
  },
  {
    ...
    tooltip: {
      title: 'Patient Discovery + Discharge Guard',
      body: 'Fetches current admitted patients per workspace. Patients no longer in the admitted list are automatically deactivated in snapshot_schedule (deactivated_reason: "discharged").',
      fields: [
        { k: 'Auth',         v: 'GCP service account ID token' },
        { k: 'Discharge',    v: 'Auto-deactivates patients no longer in admitted list' },
        { k: 'Idempotency',  v: 'Skips if a snapshot already exists within last 50 min' },
      ],
    },
  },
  {
  ...
  {
    id: 'snapshot_store', label: 'Local MongoDB', sublabel: 'snapshots collection · snapshot_at',
    lane: 'sync', type: 'artifact',
    tooltip: {
      title: 'Snapshot Store (db.snapshots)',
      body: 'Full chart stored as a timestamped document in db.snapshots. Each hourly run produces a new immutable snapshot. This is the single source of truth for all downstream steps — Radar is not called again until the next run.',
      fields: [
        { k: 'Collection',   v: 'snapshots' },
        { k: 'Key fields',   v: 'CPMRN, encounter, snapshot_at (UTC)' },
        { k: 'Content',      v: 'chart: {vitals, documents, orders, notes, io, …}' },
        { k: 'Idempotency',  v: 'Skipped if snapshot exists within last 50 min' },
      ],
    },
  },
  // ── Note Indexing lane ───────────────────────────────────────────────────
  {
    id: 'load_admission', label: 'load_admission()', sublabel: 'notes_module · from snapshot',
    lane: 'notes', type: 'process',
    tooltip: {
      title: 'Admission Note Loader',
      body: 'Reads the chart from the freshly stored snapshot. Extracts finalNotes, strips HTML from each component, and produces text chunks ready for embedding. Runs inside the hourly pipeline immediately after the snapshot is stored.',
      fields: [
        { k: 'Source',      v: 'snapshot chart.notes.finalNotes' },
        { k: 'HTML strip',  v: 'All note components HTML-stripped before chunking' },
        { k: 'Module',      v: 'tools/radar_sync/notes_module/admission_loader.py' },
      ],
    },
  },
  {
    ...
    tooltip: {
      title: 'Note Indexing',
      body: 'Notes chunked at 600 tokens (80-token overlap via tiktoken cl100k_base). Embedded with Gemini Embedding API in batches. FAISS IndexFlatL2 built over all chunks. Rebuilt fresh every hourly run from the latest snapshot.',
      fields: [
        ...
        { k: 'Rebuild',    v: 'Fresh each run — always reflects latest snapshot' },
      ],
    },
  },
  {
    ...
    tooltip: {
      ...
      body: 'FAISS index serialized via faiss.serialize_index() → stored as bson.Binary in local MongoDB. Chunks stored as JSON array. Keyed by admission_id = "{CPMRN}_{encounter}".',
      fields: [
  ...
  // ── Context Building lane ────────────────────────────────────────────────
  {
    ...
    tooltip: {
      title: 'Chart Delta',
      body: 'Reads last_snapshot_at from patient_contexts. Extracts only events newer than the previous run: vitals, labs, notes (by timestamp), and the full current orders snapshot.',
      fields: [
  ...
  {
    id: 'summary_update', label: 'Rolling Summary Update', sublabel: 'Gemini · PatientSummary',
    lane: 'context', type: 'process',
    tooltip: {
      title: 'Structured Summary Update (Gemini)',
      body: 'Calls summary_updater.update_summary() using gemini-3.1-flash-lite with structured output. Produces a PatientSummary dict — one ActiveProblem entry per clinical problem, each with a status field: critical / worsening / stable / improving / resolved.',
      fields: [
        { k: 'Model',       v: 'gemini-3.1-flash-lite (structured JSON output)' },
        { k: 'Schema',      v: 'PatientSummary: admission_narrative, problems[], resolved_problems[], narrative, suggested_actions[]' },
        { k: 'Problem',     v: '{name, status, presenting_features, workup, management, current_state, plan_changing_event}' },
        { k: 'First run',   v: 'Full chart context — initial assessment from scratch' },
        { k: 'Update runs', v: 'Delta only — preserves historical context, updates current_state' },
      ],
    },
  },
  {
    id: 'running_ctx', label: 'patient_contexts', sublabel: 'structured_summary · MongoDB',
    lane: 'context', type: 'artifact',
    tooltip: {
      title: 'Persistent Patient Context',
      body: 'MongoDB document per patient. Stores both the rich structured_summary dict (problems + statuses) and the plain running_summary narrative for backward compatibility. hourly_entries appended only when CDS fires.',
      fields: [
        { k: 'Collection',         v: 'patient_contexts' },
        { k: 'structured_summary', v: 'Full PatientSummary dict — problems with status labels' },
        { k: 'running_summary',    v: 'Plain narrative (backward compat + LLM context)' },
        { k: 'last_snapshot_at',   v: 'UTC timestamp of last successful run' },
        { k: 'hourly_entries',     v: 'Append-only log — written only when CDS triggers' },
      ],
    },
  },
  // ── CDS lane (conditional) ────────────────────────────────────────────────
  {
    id: 'worsening_check', label: 'Worsening Check', sublabel: 'problems[].status',
    lane: 'cds', type: 'decision',
    tooltip: {
      title: 'CDS Trigger — Status Gate',
      body: 'Reads the problems list from the freshly updated PatientSummary. If ANY problem has status "worsening" or "critical", CDS fires. If all problems are stable/improving/resolved, CDS and the Google Chat alert are both skipped — and this is logged.',
      fields: [
        { k: 'Trigger statuses', v: '"worsening", "critical"' },
        { k: 'Skip statuses',    v: '"stable", "improving", "resolved"' },
        { k: 'If skipped',       v: 'pipeline logs reason; no entry added to hourly_entries' },
      ],
    },
  },
  {
    id: 'cds_call', label: 'CDS Call', sublabel: 'cds_runner · wiki-grounded',
    lane: 'cds', type: 'process',
    tooltip: {
      title: 'Wiki-Grounded CDS (Conditional)',
      body: 'Calls run_cds() from cds_runner.py. Feeds the structured PatientSummary + current chart + I/O balance. Only executes when the worsening check passes.',
      fields: [
        { k: 'Function',  v: 'run_cds(cpmrn, encounter, structured_summary, chart, io_summary)' },
        { k: 'Context',   v: 'PatientSummary narrative + vitals + orders + labs + I/O balance' },
  ...
  },
  {
    ...
    tooltip: {
      ...
      body: 'Semantic search over all clinician notes. The LLM can call this on demand during the CDS call. Loads the FAISS index built earlier in this same hourly run.',
      fields: [
  ...
  {
    id: 'suggestions', label: 'Suggestions', sublabel: 'hourly_entries · conditional',
    lane: 'cds', type: 'artifact',
    tooltip: {
      title: 'CDS Output — Written Only When Triggered',
      body: 'Structured CDS result appended to patient_contexts.hourly_entries. Includes the triggering problem names, the full PatientSummary at that point, and CDS action lists. Only written when worsening_check passes.',
      fields: [
        { k: 'Storage',              v: 'patient_contexts.hourly_entries (append-only)' },
        { k: 'triggering_problems',  v: 'Names of problems with worsening/critical status' },
        { k: 'structured_summary',   v: 'PatientSummary snapshot at time of alert' },
        { k: 'suggestions',          v: 'immediate_actions, immediate_next_steps, clinical_reasoning' },
      ],
    },
  },
  {
    id: 'gchat_alert', label: 'Google Chat Alert', sublabel: 'webhook · card message',
    lane: 'cds', type: 'io',
    tooltip: {
      title: 'Google Chat Webhook Alert',
      body: 'Posts a structured card to the configured Google Chat space. Shows the worsening/critical problem names with current_state values, and the top CDS suggested actions. Only fires when the webhook is enabled in Settings → Alerts.',
      fields: [
        { k: 'Trigger',      v: 'Any problem with status worsening or critical' },
        { k: 'Format',       v: 'Google Chat card: problem blocks + suggested actions + narrative snippet' },
        { k: 'Config',       v: 'Settings → Alerts → Webhook URL + enable toggle (db.app_settings)' },
        { k: 'Module',       v: 'tools/radar_sync/gchat_notifier.py' },
        { k: 'If disabled',  v: 'Silently skipped — suggestions still written to hourly_entries' },
      ],
    },
  },
]
...
const SYNC_EDGES = [
  ...
  { from: 'chart_pull',      to: 'snapshot_store' },
  { from: 'snapshot_store',  to: 'load_admission',    cross: true },
  { from: 'load_admission',  to: 'build_faiss' },
  { from: 'build_faiss',     to: 'note_cache' },
  { from: 'snapshot_store',  to: 'delta_extract',     cross: true },
  { from: 'delta_extract',   to: 'summary_update' },
  { from: 'summary_update',  to: 'running_ctx' },
  { from: 'running_ctx',     to: 'worsening_check',   cross: true },
  { from: 'note_cache',      to: 'notes_tool',         cross: true },
  { from: 'notes_tool',      to: 'cds_call',           label: 'on demand', dashed: true, arcAbove: true },
  { from: 'worsening_check', to: 'cds_call',           label: 'worsening/critical' },
  { from: 'cds_call',        to: 'suggestions' },
  { from: 'suggestions',     to: 'gchat_alert' },
